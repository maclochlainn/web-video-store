...
|| -------------------------------------------------------------------
||  Description:
||    - Creates the update_price function, which is called from
  ...
  FOR i IN 1..flag.COUNT LOOP
    ...
    FOR j IN common_lookup LOOP
      ...
      FOR k IN item LOOP
        ...
        IF TRUNC(SYSDATE) - 30 = k.release_date AND flag(i) = 'N' THEN
          lv_start_date := TRUNC(k.release_date);
          lv_end_date := TRUNC(k.release_date) + 30;
        ELSIF TRUNC(SYSDATE) - 30 > k.release_date AND flag(i) = 'Y' THEN
          lv_start_date := TRUNC(k.release_date) + 31;
        END IF;
